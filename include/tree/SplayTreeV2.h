#ifndef SPLAYTREEV2_H
#define SPLAYTREEV2_H
  ...
  friend std::ostream& operator<<(std::ostream& out, const SplayTree& rhs) {
    ...
    while (!q.empty()) {
      ...
      if (node->mChildren[0] != nullptr) {
        q.push(node->mChildren[0]);
      }
      if (node->mChildren[1] != nullptr) {
        q.push(node->mChildren[1]);
      }
...
private:
  ...
  struct Node {
    ...
    friend std::ostream& operator<<(std::ostream& out, const Node* rhs) {
      ...
      if (rhs->mChildren[0] != nullptr) {
        out << "L" << rhs->mChildren[0]->mValue.first;
      } else {
        out << "null";
      }

      out << "_(" << rhs->mValue.first << ")";

      if (rhs->mParent != nullptr) {
        out << "P" << rhs->mParent->mValue.first;
      }

      if (rhs->mChildren[1] != nullptr) {
        out << "_R" << rhs->mChildren[1]->mValue.first;
      } else {
        out << "_null";
      }
      return out;
